    ...
    <div className="flex justify-between items-center w-full h-20 px-10 text-[#224665] bg-white">
      ...
      <ul className="hidden md:flex">
        {links.map((item) => (
          <li
            key={item.id}
            className={`px-8 mt-5 cursor-pointer capitalize font-medium text-black hover:scale-105 duration-200 `}
          >
      ...
      <div
        onClick={() => setNav(!nav)}
        className="cursor-pointer pr-4 text-black md:hidden z-40"
      >
        {nav ? <FaTimes size={30} /> : <FaBars size={30} />}
      </div>

      {nav && (
        <ul className="flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen bg-gradient-to-b from-white to-[#D8D9D8] z-10">
          {links.map((item) => (
            <li
              key={item.id}
